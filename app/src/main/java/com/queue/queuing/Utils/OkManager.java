package com.queue.queuing.Utils;
...
public class OkManager {
     ...
     * @param callBack
     */
    private void onFailJsonObjectMethod(final String jsonValue, final Fun4 callBack) {
        handler.post(new Runnable() {
            @Override
            public void run() {
                if (callBack != null) {
     ...
     * @param callback
     */
    public void sendJSONByPost(String url, String jsonData, final Fun4 callback) {
        Request request = new Request.Builder().url(url).post(RequestBody.create(JSON, jsonData)).build();
        client.newCall(request).enqueue(new Callback() {
            @Override
            public void onFailure(Call call, IOException e) {
                onFailJsonObjectMethod(e.getMessage(), callback);
            }
