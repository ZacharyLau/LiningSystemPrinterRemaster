 ...
 */
class QRKanji extends com.queue.queuing.qrcode.QRData {
	...
	public void write(BitBuffer buffer) {
		...
		try {
			...
			byte[] data = getData().getBytes(com.queue.queuing.qrcode.QRUtil.getJISEncoding() );
	...
	public int getLength() {
		try {
			return getData().getBytes(com.queue.queuing.qrcode.QRUtil.getJISEncoding() ).length / 2;
		} catch(UnsupportedEncodingException e) {
			throw new RuntimeException(e.getMessage() );
		}
	}	
