...
class QRUtil {
    ...
    public static int getMaxLength(int typeNumber, int mode, int errorCorrectLevel) {
        ...
        switch(mode) {
        case com.queue.queuing.qrcode.Mode.MODE_NUMBER    : m = 0; break;
        case com.queue.queuing.qrcode.Mode.MODE_ALPHA_NUM : m = 1; break;
        case com.queue.queuing.qrcode.Mode.MODE_8BIT_BYTE : m = 2; break;
        case com.queue.queuing.qrcode.Mode.MODE_KANJI     : m = 3; break;
        default :
	...
	public static Polynomial getErrorCorrectPolynomial(int errorCorrectLength) {
		...
		for (int i = 0; i < errorCorrectLength; i++) {
			a = a.multiply(new Polynomial(new int[]{1, com.queue.queuing.qrcode.QRMath.gexp(i) }) );
		}
	...
	public static int getMode(String s) {
		if (isAlphaNum(s) ) {
			if (isNumber(s) ) {
				return com.queue.queuing.qrcode.Mode.MODE_NUMBER;
			}
			return com.queue.queuing.qrcode.Mode.MODE_ALPHA_NUM;
		} else if (isKanji(s) ) {
			return com.queue.queuing.qrcode.Mode.MODE_KANJI;
		} else {
